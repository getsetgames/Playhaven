...
static NSString *const kPHRequestParameterLocationOptOutStatusKey = @"location_opt_out";
...
@property (nonatomic, unsafe_unretained) PHRequestStatus requestStatus;
@property (nonatomic, strong, readwrite) NSDictionary *signedParameters;
@property (nonatomic, strong, readwrite) NSURL *URL;
@property (nonatomic, strong, readonly) NSDictionary *basicParameters;
...
+ (NSMutableSet *)allNonces {
    static NSMutableSet *allNonces = nil;
    
    if (allNonces == nil) {
        allNonces = [[NSMutableSet alloc] init];
    }
...
+ (BOOL)locationOptOutStatus {
    NSNumber *pref = [[NSUserDefaults standardUserDefaults] objectForKey:@"PlayHavenLocationOptOutStatus"];
    return (pref) ? [pref boolValue] : NO;
}

+ (void)setLocationOptOutStatus:(BOOL)yesOrNo {
    [[NSUserDefaults standardUserDefaults] setBool:yesOrNo forKey:@"PlayHavenLocationOptOutStatus"];
}
...
+ (void)setPluginIdentifier:(NSString *)identifier
{
    @synchronized (self) {
        ...
        sPlayHavenPluginIdentifier = string;
    }
}
...
- (NSDictionary *)basicParameters
{
    ...
    [[PHAPIRequest allNonces] addObject:nonce];
    ...
    NSNumber *locationOptOutStatus = @([PHAdRequest locationOptOutStatus]);
    ...
    NSDictionary *signatureParams =
         [NSDictionary dictionaryWithObjectsAndKeys:
                             ...
                             locationOptOutStatus, kPHRequestParameterLocationOptOutStatusKey,
                             nil];
