...
@interface PHContentView : UIView <UIWebViewDelegate, PHURLLoaderDelegate> {
    ...
    UIWebView *_webView;
    UIWebView *_activeWebView;
    BOOL       _willAnimate;

    NSMutableDictionary     *_redirects;
    UIActivityIndicatorView *_activityView;
    PHMoviePlayerViewController *_moviePlayerController;
    BOOL _showPostVideoCTA;
}
...
@property(nonatomic, strong) PHContent *content;    /**< Sets the PHContent instance for this view, PHContent defines the
                                                         template, transition, frame size, and context for this content unit */
@property(nonatomic, weak) UIView    *targetView; /**< When shown, the content view will attempt to attach itself to this view. Defaults to nil */
@property(nonatomic, assign) BOOL   hasNoAd; /**< Set to YES in the case that an external ad call returns no ads */
@property(nonatomic, weak) id<PHContentViewDelegate> delegate; /**< Content view delegate */
