...
{
    return [[PHConnectionBundle alloc] initWithRequest:request forDelegate:delegate withContext:context];
}
...
+ (BOOL)createConnectionFromRequest:(NSURLRequest *)request forDelegate:(id <PHConnectionManagerDelegate>)delegate withContext:(id)context
{
    ...
    CFDictionaryAddValue(connectionManager.connections,
                         (__bridge const void *)(connection),
                         (__bridge const void *)(connectionBundle));
...
+ (void)stopConnectionsForDelegate:(id<PHConnectionManagerDelegate>)delegate
{
    ...
    {
        connectionBundle = (PHConnectionBundle *) CFDictionaryGetValue(connectionManager.connections, (__bridge const void *)(connection));
        ...
        if ([connectionBundle delegate] == delegate)
        {
            ...
            CFDictionaryRemoveValue(connectionManager.connections, (__bridge const void *)(connection));
        }
    }
}
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    PHConnectionBundle *connectionBundle = (PHConnectionBundle *)CFDictionaryGetValue(self.connections, (__bridge const void *)(connection));

    [connectionBundle setData:[[NSMutableData alloc] init]];
...
{
    [[(PHConnectionBundle *)CFDictionaryGetValue(self.connections, (__bridge const void *)(connection)) data] appendData:data];
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    CFDictionaryRemoveValue(self.connections, (__bridge const void *)(connection));
}
