...
typedef struct {
    //TODO: change struct to Objective-C class
    unichar character;
    __unsafe_unretained NSString *entity;
} HTMLEntityPair;
...
+ (NSString *)stringByUrlEncodingString:(NSString *)input
{
    ...
    NSString *result = [NSString stringWithString:(__bridge_transfer NSString *)value];
...
+ (NSString *)uuid
{
    ...
    if (NULL != uuid)
    {
        ...
        if (NULL != uuidRef)
        {
            result = [NSString stringWithString:(__bridge_transfer NSString *)uuidRef];
        }
    }
