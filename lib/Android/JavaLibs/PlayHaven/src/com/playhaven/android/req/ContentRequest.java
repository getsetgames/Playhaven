...
public class ContentRequest extends PlayHavenRequest
{
    ...
    protected UriComponentsBuilder createUrl(Context context) throws PlayHavenException {
        ...
        if(placementTag != null) {
            placementTag = placementTag.toLowerCase();
        ...
        if (locationManager != null &&
                PlayHaven.USE_LOCATION == true &&
                PlayHaven.getLocationOptOut(context) == false &&
                PlayHaven.isLocationEnabled(context)) {
            ...
            if (lastKnownLocationGPS != null) {
                lon = String.valueOf(lastKnownLocationGPS.getLongitude());
                lat = String.valueOf(lastKnownLocationGPS.getLatitude());
                acc = String.valueOf(Math.round(lastKnownLocationGPS.getAccuracy()));
            }
        }

        builder.queryParam("lon", lon);
        builder.queryParam("lat", lat);
        builder.queryParam("acc", acc);
        builder.queryParam("location_opt_out", PlayHaven.getLocationOptOut(context));
