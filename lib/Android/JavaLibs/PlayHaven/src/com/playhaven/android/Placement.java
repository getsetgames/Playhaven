...
implements Parcelable, CacheResponseHandler, RequestListener
{
    ...
    public void preload(final Context context)
    {
        ...
        if (PlayHaven.DID_CALL_OPEN == false) {
    ...
    /**
     * Checks if the response is coming from a OpenRequest or a Content request and handles the
     * model returned from the server call
     *
    ...
    public void handleResponse(Context context, String json) {
        // check if result from open or content
        if (JsonUtil.getPath(json, "$.response.prefix") != null) {
